...
export class SearchService {
  ...
  async search(params: { queryText: string; userLat: number; userLon: number; limit: number }) {
    ...
    let qb = this.repo
      ...
      .andWhere("public.ST_DWithin(s.location, public.ST_GeogFromGeoJSON(:userLocation), :radius)", {
    ...
    const distanceExpr = "public.ST_Distance(s.location, public.ST_GeogFromGeoJSON(:userLocation))";
  ...
  async semanticSearch(params: { queryText: string; userLat: number; userLon: number; limit: number }) {
    ...
    const qb = this.repo
      .createQueryBuilder("i")
      .leftJoinAndSelect("i.shop", "s")
      .where("public.ST_DWithin(s.location, public.ST_GeogFromGeoJSON(:userLocation), :radius)", {
        userLocation: JSON.stringify(userLocation),
        radius: SEARCH_RADIUS_METERS,
      });

    // Add distance calculation and order by vector similarity (cosine distance)
    qb.addSelect("public.ST_Distance(s.location, public.ST_GeogFromGeoJSON(:userLocation))", "distance_m");
